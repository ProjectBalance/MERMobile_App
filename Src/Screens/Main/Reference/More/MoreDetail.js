...
import {View,Text,ScrollView,Linking, StyleSheet,TouchableHighlight,NetInfo,ActivityIndicator} from 'react-native';
...
export default class MoreDetail extends Component{
    ...
    return {
        title:navigation.getParam("group",""),
        headerStyle: {
    ...
    componentDidMount(){
        ...
        if(o[0].link != ""){
            link = <Text style={{color:'blue', marginTop:10, fontSize:TextFactory.GetFontSize(15)}} onPress={() => Linking.openURL(GeneralFactory.CheckURL(o[0].link))}>
                {o[0].link}
            </Text>
        }
            ...
            <View style={{paddingTop:20, paddingLeft:10, paddingRight:10, backgroundColor:'#FFF', flex:1}}>
                <View style={{marginBottom:20}}>
                    <Text style={{fontSize:TextFactory.GetFontSize(20), fontWeight:'bold'}}>
                        {this.props.navigation.getParam("title","More Detail")}
                    </Text>
                </View>
                <ScrollView>
